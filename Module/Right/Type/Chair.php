...
<div class="sanpham">
    <div class="pagination">
        <?php if ($trangso > 2) { ?>
            <a href="index.php?xem=Chair&perpage=<?= $sosanpham ?>&page=<?= 1 ?>">FIRST</a> <?php } ?>
        <?php if ($trangso < $sotrang + 1) { ?>
            <?php $nextpage = $trangso - 1; ?>
            <a href="index.php?xem=Chair&perpage=<?= $sosanpham ?>&page=<?= $nextpage ?>">PREV</a> <?php } ?>
        <?php for ($num = 1; $num <= $sotrang; $num++) { ?>
            <?php if ($num != $trangso) { ?>
                <?php if ($num > $trangso - 2 and $num < $trangso + 2) { ?>
                    <a href="index.php?xem=Chair&perpage=<?= $sosanpham ?>&page=<?= $num ?>"><?= $num ?></a>
                <?php } ?>
            <?php } else { ?>
        ...
        <?php if ($trangso < $sotrang) { ?>
            <?php $nextpage = $trangso + 1; ?>
            <a href="index.php?xem=Chair&perpage=<?= $sosanpham ?>&page=<?= $nextpage ?>">NEXT</a> <?php } ?>
        <?php if ($trangso < $sotrang - 1) { ?>
            <a href="index.php?xem=Chair&perpage=<?= $sosanpham ?>&page=<?= $sotrang ?>">LAST</a> <?php } ?>
    </div>
</div>