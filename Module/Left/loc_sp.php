<div class="content_locsp">
    <p class="head-filter">
        <strong>
            Lọc theo hãng sản xuất
        </strong>
    </p>
    <ul>
        <li>
            <a href="index.php?xem=HomeOff&id=1"><div class="filter"><strong>HomeOff</strong></div></a>
        </li>
        <li>
            <a href="index.php?xem=Prohome-Moho&id=1"><div class="filter"><strong>Prohome - Moho</strong></div></a>
        </li>
        <li>
            <a href="index.php?xem=Penny-Ahura&id=1"><div class="filter"><strong>Penny - Ahura</strong></div></a>
        </li>
        <li>
            <a href="index.php?xem=Fence-Jazz&id=1"><div class="filter"><strong>Fence - Jazz</strong></div></a>
        </li>
        <li>
            <a href="index.php?xem=AnotherBrand&id=1"><div class="filter"><strong>Các hãng khác</strong></div></a>
        </li>
    </ul>
    <p class="head-filter">
        <strong>
            Lọc theo màu sắc
        </strong>
    </p>
    <ul>
        <li>
            <a href="index.php?xem=White&id=1"><div class="filter"><strong>Trắng</strong></div></a>
        </li>
        <li>
            <a href="index.php?xem=Gray&id=1"><div class="filter"><strong>Đen</strong></div></a>
        </li>
        <li>
            <a href="index.php?xem=Brown&id=1"><div class="filter"><strong>Nâu</strong></div></a>
        </li>
        <li>
            <a href="index.php?xem=Blue&id=1"><div class="filter"><strong>Xanh</strong></div></a>
        </li>
        <li>
            <a href="index.php?xem=AnotherColor&id=1"><div class="filter"><strong>Các màu khác</strong></div></a>
        </li>
    </ul>
    <p class="head-filter">
        <strong>
            Lọc theo loại sản phẩm
        </strong>
    </p>
    <ul>
    <ul>
        <li>
            <a href="index.php?xem=Sofa&id=1"><div class="filter"><strong>Sofa</strong></div></a>
        </li>
        <li>
            <a href="index.php?xem=Table&id=1"><div class="filter"><strong>Bàn</strong></div></a>
        </li>
        <li>
            <a href="index.php?xem=Bed&id=1"><div class="filter"><strong>Giường</strong></div></a>
        </li>
        <li>
            <a href="index.php?xem=Chair&id=1"><div class="filter"><strong>Ghế</strong></div></a>
        </li>
        <li>
            <a href="index.php?xem=ArmChair&id=1"><div class="filter"><strong>Arm Chair</strong></div></a>
        </li>
        <li>
            <a href="index.php?xem=AnotherType&id=1"><div class="filter"><strong>Các loại khác</strong></div></a>
        </li>
    </ul>
    </ul>
</div>